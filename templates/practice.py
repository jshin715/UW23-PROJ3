...
# Database Setup
#################################################
database_url1 = f"postgresql://{username}:{password}@{host}:{port}/{database}"
...
def table():
    ...
    """Return data from the covtable."""
    # Query the covtable
    results = session.query(Covtable.state, Covtable.county, Covtable.fips_code,Covtable.county_population,
